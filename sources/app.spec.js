	...
	describe("Criar uma série - createSerie", function() {
		...
		it('Informacoes nok - equipe legenda parceira vazio', function(done) {
			...
			.end(function(err, res) {
				...
				expect(ret.msg).to.have.string(eMV_S.eq_leg_parc)
	...
	describe("Alterar uma série - updParcialSerie", function() {
		...
		it('Validar alterações', function(done) {
			...
			.end(function(err, res) {
				...
				expect(res.body).to.have.all.keys(oM_S.arrayKeys)
				expect(res.body).to.not.have.all.keys([oM_S.param.v])
				expect(res.body.nome).to.be.equal(oUP_S.serieOk.nome)
				expect(res.body.sp).to.be.null
				expect(res.body.sm).to.be.null
				expect(res.body.hia).to.be.null
				expect(res.body.sf).to.be.null
				expect(res.body.sinalizador).to.be.equal(oUP_S.serieOk.sinalizador)
				expect(res.body.status).to.be.equal(oUP_S.serieOk.status)
				expect(res.body.dia).to.be.equal(oUP_S.serieOk.dia)
